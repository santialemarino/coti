 ...
 *
 * It does not exist in a build. Nothing links here, but "unlinked" is not "unreachable", and a
 * seller who guesses the path should not land on a developer's bench full of English labels. Same
 * shape as the API's own dev-only route, which the router hangs behind `!cfg.IsProduction()`.
 */
// Built as well as rendered: a page that answers 404 must not name itself in the browser tab.
export function generateMetadata(): Metadata {
  if (process.env.NODE_ENV === 'production') return {};
  return { title: 'Design system — Coti', robots: { index: false, follow: false } };
}
