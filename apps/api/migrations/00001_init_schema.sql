-- Coti — esquema inicial. Release 1 (Piloto Comercial).
--
-- Convenciones: identificadores en inglés, comentarios en español. PK UUID v4.
-- Plata y cantidades en NUMERIC(14,2). Enums nativos con valores en inglés
-- UPPERCASE (los labels en español viven en el i18n del frontend).
--
-- Toda tabla con dueño de tenant lleva account_id, incluidas las hijas: hace que
-- cada política de RLS y cada WHERE sea un predicado plano en vez de un join
-- hacia arriba.
...
DROP TABLE IF EXISTS notification, handler_decision, quote_discount_item, quote_discount,
  promotion_tier, promotion_condition_item, promotion, quote_message, message_batch,
  client_action, quote_send, quote_status_change, quote_item_alternative, quote_item,
  quote_version, quote, rfq_status_change, rfq_attachment, rfq, channel, client_tag, tag,
  client, combo_item, combo, product_alternative, product_price, product_synonym,
  branch_product, product, refresh_token, user_branch, app_user, branch, account CASCADE;

DROP FUNCTION IF EXISTS app_current_account_id();
DROP FUNCTION IF EXISTS set_updated_at();

DROP TYPE IF EXISTS message_batch_status, message_author_type, notification_status,
  handler_seller_decision, send_tracking_status, send_format, attachment_processing_status,
  attachment_type, channel_type, discount_origin, discount_scope, promotion_action_type,
  promotion_condition_type, client_action_type, quote_item_alternative_origin,
  quote_item_alternative_type, product_alternative_type, item_match_status, user_role,
  quote_status, rfq_status;
