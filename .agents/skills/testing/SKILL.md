...
- **Teardown ordering is a rule, not a detail.** `t.Cleanup` runs **after** the test body's `defer`s,
  ...
  failed delete leaves rows behind and the suite still passes. `mustCleanup` fails the test when a
  delete cannot happen; use it for every delete in a teardown.
