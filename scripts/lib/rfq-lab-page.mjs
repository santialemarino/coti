...
<body>
  ...
  <script>
    let state = { types: [], cases: [], reports: [], api_online: false };
    ...
    function renderSelection() {
      ...
      $('#run-help').textContent = selectedType.uses_ai ? 'Requiere confirmación de consumo' : 'Sin consumo de IA';
    }
    ...
    function statusText(status) {
      return { RUNNING: 'EJECUTANDO', PASSED: 'APROBADO', FAILED: 'FALLÓ' }[status] || status;
    }

    function setRunning(running) {
      $('#run-main').disabled = running;
      $('#run-top').disabled = running;
